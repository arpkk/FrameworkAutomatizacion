    ...
    stage('Xray') {
      steps {
        echo "xray"
          ...
          curl -H "Content-Type: multipart/form-data" -X POST -F info=@"Info.json" -F -H "Authorization: Bearer $token" https://xray.cloud.getxray.app/api/v2/import/execution/cucumber/multipart
          '''
      }
    }
